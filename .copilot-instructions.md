...
This is a **RAG (Retrieval-Augmented Generation) backend** built with **FastAPI**, backed by **AWS S3**, **Amazon OpenSearch** (hybrid search), and an **agent layer** (Google ADK + MCP). The codebase follows a strict layered architecture. All new functionality must follow the step-by-step implementation workflow described below.
